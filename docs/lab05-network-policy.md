...
jq -e '.spec.template.spec.containers |
  any(.[]; .name == "aws-network-policy-agent" and
    any(.args[]; . == "--enable-network-policy=true")) and
...
kubectl -n kube-system logs -l k8s-app=aws-node -c aws-network-policy-agent \
...
policy-agent container on each. Inspect the captured schema for these settings,
